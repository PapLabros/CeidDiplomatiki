...
namespace CeidDiplomatiki
{
    ...
    /// </summary>
    public class MySQLProviderIndexColumn : IDbProviderIndex
    {
        ...
        /// Default constructor
        /// </summary>
        internal MySQLProviderIndexColumn(DataRow row)
        {
            IndexCatalog = row.GetDbNullableString(0);
            IndexSchema = row.GetString(1);
            IndexName = row.GetString(2);
            TableName = row.GetString(3);
            ColumnName = row.GetString(4);
            OrdinalPosition = row.GetInt(5);
            SortOrder = row.GetString(6);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public override int GetHashCode() => HashCode.Combine(IndexCatalog, IndexSchema, IndexName, TableName, ColumnName, OrdinalPosition, SortOrder);
